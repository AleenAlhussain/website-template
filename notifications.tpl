{include file='_head.tpl'}
{include file='_header.tpl'}

<!-- page content -->
<div class="container mb30">
	<!-- content panel -->
	<div class="row">
		<!-- center panel -->
			<div class="col-lg-8 middlecol">
				<!-- notifications -->
				<div class="ele_content">
					<h2 class="ele_page_title small">{__("Your Notifications")}</h2>
					
					{if $user->_data['notifications']}
						<button class="btn btn-mat btn-danger js_delete-all-notifications">
							{__("Delete All")}
						</button>
					{/if}
					
					<div class="card-body pt-0 px-0">
						{if $user->_data['notifications']}
							<ul>
								{foreach $user->_data['notifications'] as $notification}
									{include file='__feeds_notification.tpl' _tpl="with-action"}
								{/foreach}
							</ul>

							{if count($user->_data['notifications']) >= $system['max_results']}
								<!-- see-more -->
								<div class="alert alert-info see-more js_see-more" data-get="notifications">
									<span>{__("See More")}</span>
									<div class="loader loader_small x-hidden"></div>
								</div>
								<!-- see-more -->
							{/if}
						{else}
							<p class="text-center text-muted empty_state">
								<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m416 512h-320c-53.023438 0-96-42.976562-96-96v-320c0-53.023438 42.976562-96 96-96h320c53.023438 0 96 42.976562 96 96v320c0 53.023438-42.976562 96-96 96zm0 0" fill="#e3f8fa"></path><path d="m245.328125 384c19.3125 0 35.472656-13.761719 39.183594-32h-78.382813c3.726563 18.238281 19.886719 32 39.199219 32zm0 0" fill="#8ce1eb"></path><path d="m320.128906 256c-.050781 0-.082031 0-.128906 0-41.167969 0-74.671875-33.488281-74.671875-74.671875 0-11.3125 2.609375-22.015625 7.136719-31.648437-2.351563-.222657-4.722656-.367188-7.136719-.367188-41.230469 0-74.671875 33.421875-74.671875 74.671875v29.742187c0 21.105469-9.25 41.027344-25.472656 54.753907-5.40625 4.625-7.808594 11.984375-5.871094 19.152343 2.289062 8.367188 10.527344 13.695313 19.199219 13.695313h173.601562c9.085938 0 17.664063-5.886719 19.503907-14.785156 1.421874-6.878907-1.023438-13.773438-6.371094-18.253907-15.519532-13.023437-24.476563-32.128906-25.117188-52.289062zm0 0" fill="#26c6da"></path><path d="m373.328125 181.328125c0 29.453125-23.875 53.328125-53.328125 53.328125s-53.328125-23.875-53.328125-53.328125 23.875-53.328125 53.328125-53.328125 53.328125 23.875 53.328125 53.328125zm0 0" fill="#8ce1eb"></path></svg> {__("No notifications")}
							</p>
						{/if}
					</div>
				</div>
				<!-- notifications -->
			</div>
		<!-- center panel -->

		<!-- right panel -->
			<div class="col-lg-4 sidebar rightcol js_sticky-sidebar">
				{include file='_ads_campaigns.tpl'}
				{include file='_ads.tpl'}
				{include file='_widget.tpl'}
			</div>
		<!-- right panel -->
	</div>
	<!-- content panel -->
</div>
<!-- page content -->

{include file='_footer.tpl'}