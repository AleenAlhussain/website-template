...
<div class="ele_side_backdrop" onclick="javascript:$('body').toggleClass('side_open');"></div>
...
<script>
    ...
    {if $user->_logged_in}
      ...
      {if $system['ai_text_credit_enabled'] || $system['ai_image_credit_enabled']}
        var ai_credit_cost = "{$system['ai_credit_cost']}";
      {/if}
    {/if}
  </script>
  <script>
    /* i18n for JS */
    var __ = [];
	__['What is on your mind? #Hashtag.. @Mention.. Link..'] = `{__('What is on your mind? #Hashtag.. @Mention.. Link..')}`;
...
{else}
    <script src="https://cdn.rtlcss.com/bootstrap/v4.5.3/js/bootstrap.min.js" integrity="sha384-VmD+lKnI0Y4FPvr6hvZRw6xvdt/QZoNHQ4h5k0RL30aGkR9ylHU56BzrE2UoohWK" crossorigin="anonymous">
...
<script src="https://cdn.plyr.io/3.8.4/plyr.js" {if !$user->_logged_in}defer{/if}>
...
{if $user->_logged_in}
    ...
    <!-- Stories -->
    {if $page == "index" && $view == ""}
        <!-- <script src="{$system['system_url']}/content/themes/{$system['theme']}/js/zuck.js"></script> -->
    ...
    <script src="https://cdn.jsdelivr.net/npm/quill-mention@6.1.1/dist/quill.mention.min.js"></script>
    ...
    {if $system['audio_call_enabled'] || $system['video_call_enabled']}
      {if $system['audio_video_provider'] == "twilio"}
        <script src="https://sdk.twilio.com/js/video/releases/2.34.0/twilio-video.min.js" defer></script>
      {/if}
    ...
    <!-- Datatables -->
	{if $page == "admin"}
		<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/2.3.7/css/dataTables.bootstrap5.min.css" defer />
		<script src="https://cdn.datatables.net/2.3.7/js/dataTables.js" defer></script>
		<script src="https://cdn.datatables.net/2.3.7/js/dataTables.bootstrap5.min.js" defer></script>
	{/if}
	{if in_array($page, ["ads", "wallet", "developers", "settings", "page", "group"])}
		<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/v/bs4/dt-1.10.21/datatables.min.css" defer />
        <script src="https://cdn.datatables.net/v/bs4/dt-1.10.21/datatables.min.js" defer></script>
    {/if}
    <!-- Datatables -->
...
<!-- Movies [JS] -->
{if $page == "movies"}
	<script src="{$system['system_url']}/content/themes/{$system['theme']}/js/jquery.adaptive-backgrounds.js"></script>
	<script>
	...
	$('.movie_poster img').on('ab-color-found', function(ev,payload){
		$('.movie_poster_shadow').css("background", "linear-gradient(90deg, " + payload.color + ", transparent)");
		$('.ele_wm_play').css("color", "" + payload.color + "");
	});
	</script>
{/if}
<!-- Movies [JS] -->
...
document.addEventListener('DOMContentLoaded', function () {
	const slider = document.querySelector('.ele_post_filters_nav');
...
<script type="text/javascript">
$(document).ready(function() {
  // When focusing or typing in the comment box
  $('textarea.js_post-comment').on('focus input', function() {
    $('.dropup.ele_create_dropup.tag_chat_header').hide();
  });

  // When focus is lost from the comment box
  $('textarea.js_post-comment').on('blur', function() {
    $('.dropup.ele_create_dropup.tag_chat_header').show();
  });
});
</script>
{/if}
