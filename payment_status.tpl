	...
	<div class="valign notfound-wrapper">
		<div class="text-center notfound">
			{if $view == "pending"}
				...
				<p>{__("Your payment is pending and will be processed soon")}. {__("You will get a notification once the payment is processed")}</p>
			{elseif $view == "failure"}
				...
				<p>{__("Your payment has been failed")}. {__("Please try again or contact us for more information")}</p>
			{/if}
			<a class="btn btn-mat btn-primary" href="{$system['system_url']}">{__("Back to homepage")}</a>
		</div>
	</div>
<!-- page content -->
